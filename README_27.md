...
После завершения работы над приложением вы можете опубликовать его в интернете, используя GitHub Pages. Это бесплатный хостинг для статических сайтов от GitHub.

### Простой способ (рекомендуется): использование пакета gh-pages

Это самый простой и быстрый способ деплоя, аналогичный React-приложениям.
...
1. [x] Приложение генерирует случайные палитры
2. [x] Цвета отображаются с HEX-значениями
3. [x] Работает копирование в буфер обмена
4. [x] Палитра сохраняется между перезагрузками
5. [x] Интерфейс интуитивно понятен
